...
sphere[:, 4:7] = sphere[:, 4:7] * np.random.rand(n, 3) * 50  # giving each sphere random positions from 0-100

def force():
    r_sphere = np.linalg.norm(sphere[:, 4:7], axis=1)
    r_black = np.linalg.norm(black[:, 4:7], axis=1)
    f_mag = np.zeros(n)
    f_vec = np.zeros((n, 3))
    for i in range(n_0):
